...
class BasePage:
    ...
    def wait_element_to_be_visible(self, loc, img_doc, timeout=20, frequency=1,):
        '''
        等待元素可见
        ...
        '''
        try:
            HandleLogger().info("开始等待页面元素<{}>是否可见！".format(loc))
            start_time = time.time()
            WebDriverWait(self.driver, timeout, frequency).until(EC.presence_of_element_located(loc))
        except Exception as e:
            HandleLogger().error("页面元素<{}>等待可见失败！".format(loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            end_time = time.time()
            HandleLogger().info("页面元素<{}>等待可见，等待时间：{}秒".format(loc, round(end_time - start_time, 2)))
    ...
    def wait_element_to_be_click(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("开始等待页面元素<{}>是否可点击！".format(loc))
            start_time = time.time()
            WebDriverWait(self.driver, timeout, frequency).until(EC.element_to_be_clickable(loc))
        except Exception as e:
            HandleLogger().error("页面元素<{}>等待可点击失败！".format(loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            end_time = time.time()
            HandleLogger().info("页面元素<{}>等待可点击，等待时间：{}秒".format(loc, round(end_time - start_time, 2)))
    ...
    def wait_element_to_be_exist(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("开始等待页面元素<{}>是否存在！".format(loc))
            start_time = time.time()
            WebDriverWait(self.driver, timeout, frequency).until(EC.presence_of_element_located(loc))
        except Exception as e:
            HandleLogger().error("页面元素<{}>等待存在失败！".format(loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            end_time = time.time()
            HandleLogger().info("页面元素<{}>等待存在，等待时间：{}秒".format(loc, round(end_time - start_time, 2)))
    ...
    def save_screenshot(self, img_doc):
        ...
        HandleLogger().info("页面截图文件保存在：{}".format(file_name))
    ...
    def get_element(self, loc, img_doc):
        ...
        :return: WebElement对象
        '''
        HandleLogger().info("在{}中查找元素<{}>".format(img_doc, loc))
        try:
            ele = self.driver.find_element(*loc)
        except Exception as e:
            HandleLogger().error("在{}中查找元素<{}>失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            return ele
    ...
    def get_elements(self, loc, img_doc):
        ...
        :return: WebElement对象
        '''
        HandleLogger().info("在{}中查找所有元素<{}>".format(img_doc, loc))
        try:
            ele = self.driver.find_elements(*loc)
        except Exception as e:
            HandleLogger().error("在{}中查找所有元素<{}>失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            return ele

    def input_text(self, text, loc, img_doc, timeout=20, frequency=1):
        '''
        对输入框输入文本内容
        :param text: 输入的文本内容
        :param loc: 元素定位的XPATH元组表达式
    ...
    def clear_text(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}中清除元素<{}>的文本内容".format(img_doc, loc))
            self.wait_element_to_be_click(loc, img_doc, timeout, frequency)
            self.get_element(loc, img_doc).clear()
        except Exception as e:
            HandleLogger().error("在{}中清除元素<{}>的文本内容失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
    ...
    def click_button(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}中点击元素<{}>".format(img_doc, loc))
            self.wait_element_to_be_click(loc, img_doc, timeout, frequency)
            self.get_element(loc, img_doc).click()
        except Exception as e:
            HandleLogger().error("在{}中点击元素<{}>失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
    ...
    def get_element_text(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}中获取元素<{}>的文本值".format(img_doc, loc))
            self.wait_element_to_be_visible(loc, img_doc, timeout, frequency)
            text = self.get_element(loc, img_doc).text
        except Exception as e:
            HandleLogger().error("在{}中获取元素<{}>的文本值失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            HandleLogger().info("获取到的元素文本值为：{}".format(text))
            return text
    ...
    def get_elements_text(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}中获取元素<{}>的所有文本值".format(img_doc, loc))
            self.wait_element_to_be_visible(loc, img_doc, timeout, frequency)
            ...
            HandleLogger().error("在{}中获取元素<{}>的所有文本值失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            HandleLogger().info("获取到的元素文本值列表为：{}".format(text_list))
            return text_list
    ...
    def get_element_attr(self, attr_name, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}中获取元素<{}>的属性{}的值".format(img_doc, loc, attr_name))
            self.wait_element_to_be_exist(loc, img_doc, timeout, frequency)
            value = self.get_element(loc, img_doc).get_attribute(attr_name)
        except Exception as e:
            HandleLogger().error("在{}中获取元素<{}>的属性{}的值失败！".format(img_doc, loc, attr_name))
            self.save_screenshot(img_doc)
            raise e
        else:
            HandleLogger().info("获取到的元素属性{}的值为{}".format(attr_name, value))
            return value
    ...
    def switch_to_frame(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}中根据元素<{}>进行iframe切换".format(img_doc, loc))
            start_time = time.time()
            WebDriverWait(self.driver, timeout, frequency).until(EC.frame_to_be_available_and_switch_to_it(loc))
        except Exception as e:
            HandleLogger().error("在{}中根据元素<{}>进行iframe切换失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            end_time = time.time()
            HandleLogger().info("在{}中根据元素<{}>进行iframe切换，等待时间：{}秒".
    ...
    def switch_to_default_content(self, img_doc):
        ...
        '''
        try:
            HandleLogger().info("切换iframe到main页面")
            self.driver.switch_to.default_content()
        except Exception as e:
            HandleLogger().error("切换iframe到main页面失败！")
            self.save_screenshot(img_doc)
            raise e
    ...
    def switch_to_parent(self, img_doc):
        ...
        '''
        try:
            HandleLogger().info("切换iframe到上一层页面")
            self.driver.switch_to.parent_frame()
        except Exception as e:
            HandleLogger().error("切换iframe到上一层页面失败！")
            self.save_screenshot(img_doc)
            raise e
    ...
    def upload_file(self, filename, img_doc, browser_type="chrome"):
        ...
        '''
        try:
            HandleLogger().info("上传文件（{}）".format(filename))
            time.sleep(2)
            upload(filePath=filename, browser_type=browser_type)
        except Exception as e:
            HandleLogger().error("上传文件（{}）失败！".format(filename))
            self.save_screenshot(img_doc)
            raise e
        else:
            time.sleep(2)
    ...
    def suspend_mouse(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}上根据元素<{}>进行悬浮".format(img_doc, loc))
            self.wait_element_to_be_click(loc, img_doc, timeout, frequency)
            ele = self.get_element(loc, img_doc)
            ActionChains(self.driver).move_to_element(ele).perform()
        except Exception as e:
            HandleLogger().error("在{}上根据元素<{}>进行悬浮失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
    ...
    def alert_close(self, img_doc, alert_type='alert', text=None, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}中切换并关闭{}类型的弹框".format(img_doc, alert_type))
            start_time = time.time()
            ...
            if alert_type in ['alert', 'confirm']:
                ...
                HandleLogger().error("不支持{},请确认alert的类型".format(alert_type))
        except Exception as e:
            HandleLogger().error("在{}中切换并关闭{}类型的弹框失败！".format(img_doc, alert_type))
            self.save_screenshot(img_doc)
            raise e
        else:
            end_time = time.time()
            HandleLogger().info("在{}中切换并关闭{}类型的弹框，等待时间：{}秒".
    ...
    def select_action(self, loc, img_doc, content, select_type, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}上根据元素<{}>以{}方式进行下拉选择".format(img_doc, loc, select_type))
            self.wait_element_to_be_click(loc, img_doc, timeout, frequency)
            ele = self.get_element(loc, img_doc)
            if select_type == 'index':
                ...
                HandleLogger().error("不支持{},请确认Select的类型".format(select_type))
        except Exception as e:
            HandleLogger().error("在{}上根据元素<{}>以{}方式进行下拉选择失败！".format(img_doc, loc, select_type))
            self.save_screenshot(img_doc)
            raise e
    ...
    def switch_to_windows(self, loc, img_doc, timeout=20, frequency=0.5):
        ...
        '''
        try:
            HandleLogger().info("在{}中根据元素<{}>进行窗口切换".format(img_doc, loc))
            ...
            HandleLogger().error("在{}中根据元素<{}>进行窗口切换失败！".format(img_doc, loc))
            self.save_screenshot(img_doc)
            raise e
        else:
            end_time = time.time()
            HandleLogger().info("在{}中根据元素<{}>进行窗口切换，等待时间：{}秒".
                             format(img_doc, loc, round(end_time - start_time, 2)))